...
std::vector<Flight> GenerateFlights(const std::vector<std::vector<int>>& graph,
                                    int hours_in_cycle,
                                    int hour_size,
                                    int min_distance,
                                    int max_distance,
    ...
    for (int i = 0; i < graph.size(); ++i) {
        for (int j = i + 1; j < graph.size(); ++j) {
            ...
            std::uniform_int_distribution<int> departure_time_dist(hour_size, hours_in_cycle - flight_time - hour_size);
...
std::vector<Aircraft> GenerateAircrafts(const std::vector<Fleet>& fleets, int aircrafts_number) {
    ...
    for (const auto& fleet : fleets) {
        for (int i = 0; i < fleet.aircrafts_number; ++i) {
            aircrafts.push_back({id, fleet.seats, fleet.flight_cost});
            id += 1;
        }
    }
    return aircrafts;
...
int main(int argc, char** argv) {
    ...
    Parser parser;
    Poco::JSON::Object::Ptr pObject = parser.parse(json_string).extract<Object::Ptr>();
    ...
    std::ifstream read_graph("graph.txt");
    ...
    auto flights = GenerateFlights(graph,
                                   hours_in_cycle,
                                   hour_size,
                                   min_distance,
                                   max_distance,
